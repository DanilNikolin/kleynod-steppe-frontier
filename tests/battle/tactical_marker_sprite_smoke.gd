...
func run() -> void:
	...
	# 1. Structure check: 9 canonical layers exist, each has child "Sprite" of type Sprite2D with non-null texture
	...
	# 2. Primitive removal check: exactly 0 Polygon2D and 0 Line2D in the scene
	...
	# 3. Preserved transforms check: Selected scale 1.08, Hover scale 1.2
	var selected_node := marker.get_node_or_null("Selected") as Node2D
	check(selected_node != null and selected_node.scale.is_equal_approx(Vector2(1.08, 1.08)),
		"Selected preserved scale (1.08, 1.08)")
	var hover_node := marker.get_node_or_null("Hover") as Node2D
	check(hover_node != null and hover_node.scale.is_equal_approx(Vector2(1.2, 1.2)),
		"Hover preserved scale (1.2, 1.2)")

	# 4. Single flag visibility check
	for layer_name in BattleTacticalMarkerView.LAYERS:
	...
	# 5. Zero flags test
	marker.set_flags(0)
	check(marker.visible == false, "Zero flags makes marker invisible")

	# 6. Combined flags test
	...
	# 8. Runtime overlay test on 6x3 arena (18 anchors)
	...
	for coord in overlay._markers:
		...
		var reach_sprite := m.get_node_or_null("Reachable/Sprite") as Sprite2D
		check(reach_sprite != null, "Reachable/Sprite exists in runtime overlay instance at " + str(coord))

	# 9. Deep forest environment integration check
