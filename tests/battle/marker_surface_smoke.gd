...
func run() -> void:
	...
	screen.tactical_state.add_state(coordinate, BattleTacticalState.Kind.REACHABLE)
	...
	screen.tactical_state.set_hover(coordinate)
	check(marker.get_node("Hover").visible and marker.get_node("Reachable").visible and marker.get_node("AoE").visible and marker.get_node("ValidTarget").visible, "Flags coexist")
	...
	check(not marker.visible, "SURFACE alone has no tactical visual")
